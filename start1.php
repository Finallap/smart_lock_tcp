<?php
	...
	{
		$input_array = database_output($data);
		...
		database_input($data,$find_result);
	...
	function database_output($data)
	{
		...
		$information=$pdo->prepare('SELECT * FROM `tcp_data` LIMIT 0,3;');
		$information->execute();
		...
		$input_array[1] = $result['content'];

		$result = $information->fetch();
		$input_array[2] = $result['content'];

		$result = $information->fetch();
		$input_array[3] = $result['content'];
